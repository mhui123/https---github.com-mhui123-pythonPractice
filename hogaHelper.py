    ...
    def receive_trdata(self, screenNo, rqName, trCode, recordName, preNext):
        ...
        if rqName == "opt10001_req":
            ...
            for i in range(nCnt):
                ...
                template["qty"] = self.getCommData(trCode, rqName, "보유수량", i).strip()
                template["avgPrice"] = self.getCommData(trCode, rqName, "평균단가", i).strip()
                template["nowPrice"] = self.getCommData(trCode, rqName, "현재가", i).strip()
                template["evalPrice"] = self.getCommData(trCode, rqName, "평가금액", i).strip()
                template["earnPrice"] = self.getCommData(trCode, rqName, "손익금액", i).strip()
                template["earnRate"] = self.getCommData(trCode, rqName, "손익율", i).strip()
                template["loanDate"] = self.getCommData(trCode, rqName, "대출일", i).strip()
                template["boughtTotal"] = self.getCommData(trCode, rqName, "매입금액", i).strip()
                template["paymentBalance"] = self.getCommData(trCode, rqName, "결제잔고", i).strip()
                
                print(f"[test] : {template})")
                self.myAssetInfo.append(template)
            print(f"[test] self.myAssetInfo: {self.myAssetInfo})")
...
class NewWindow(QWidget):
    ...
    def handle_tbItem_click(self, item):
        ...
        if col == 2 :
            ...
            text = int(re.sub(r'[,]', '', itemCheck))
            toPassData = {"hoga_interval" : self.hoga_interval, "price" : text, "hogawinData":self.hogawin_data, "myUsableCash":self.myUsableCash}
...
class HogaOrderWin(QWidget):
    ...
    def initUI(self, parent_data):
        ...
        self.tableWidget.setColumnWidth(3, int(self.tableWidget.width() * 0.2))
        self.tableWidget.setColumnWidth(4, int(self.tableWidget.width() * 0.2))
        self.tableWidget.setStyleSheet("QTableWidget { border : none; gridline-color: white; border-top:10px}") #테두리제거
        ...
        if self.account_checked == True:
            ...
            self.myUsableCash = parent_data['myUsableCash']
